...
export class SQLiteSyncRepository implements ISyncRepository {
  ...
  async getStats(tx?: DatabaseAdapter): Promise<{
    ...
    const pendingRow = await client.queryOne<{ count: number }>(
      "SELECT COUNT(*) as count FROM sync_jobs WHERE status = 'pending'"
    );
    const pendingJobs = pendingRow ? pendingRow.count : 0;

    const failedRow = await client.queryOne<{ count: number }>(
      "SELECT COUNT(*) as count FROM sync_jobs WHERE status = 'failed'"
    );
    const failedJobs = failedRow ? failedRow.count : 0;

    const lastJobRow = await client.queryOne<{ updated_at: string }>(
      "SELECT updated_at FROM sync_jobs WHERE status = 'completed' ORDER BY updated_at DESC LIMIT 1"
    );
    const lastSync = lastJobRow ? lastJobRow.updated_at : "Never";
